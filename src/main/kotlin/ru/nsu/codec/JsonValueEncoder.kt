...
internal class JsonValueEncoder(
        ...
        return when (value) {
            ...
            is Map<*, *> -> {
                ...
                when {
                    ...
                    metadataResolver.isPersistable(clazz) -> encodeObject(value, context)
    ...
    private fun encodeObject(value: Any, context: SerializationContext): ObjectNode {
        ...
        if (existingId != null) {
            return objectMapper.createObjectNode().put(JsonCodec.REF_FIELD, existingId)
        }

        val meta = metadataResolver.getMeta(value.javaClass)
        ...
        val objectNode = objectMapper.createObjectNode()
        objectNode.put(JsonCodec.ID_FIELD, objectId)
        objectNode.put(JsonCodec.VERSION_FIELD, writeVersion)
