...
internal class JsonValueDecoder(
    ...
    private val objectInstantiator = ObjectInstantiator(metadataResolver)
    ...
    private fun decode(node: JsonNode, type: Type, context: DeserializationContext): Any? {
        ...
        if (node.isObject && node.has(JsonCodec.REF_FIELD)) {
            val referenceId = node.get(JsonCodec.REF_FIELD)?.asText()
                ?: throw DeserializationException("Reference node must contain a non-null ${JsonCodec.REF_FIELD} value")
        ...
        return when {
            ...
            raw == Char::class.java || raw == Char::class.javaObjectType -> decodeChar(node)
            raw == Boolean::class.javaPrimitiveType || raw == Boolean::class.javaObjectType -> node.asBoolean()
            raw == Byte::class.javaPrimitiveType || raw == Byte::class.javaObjectType -> node.numberValue().toByte()
            raw == Short::class.javaPrimitiveType || raw == Short::class.javaObjectType -> node.numberValue().toShort()
            raw == Int::class.javaPrimitiveType || raw == Int::class.javaObjectType -> node.numberValue().toInt()
            raw == Long::class.javaPrimitiveType || raw == Long::class.javaObjectType -> node.numberValue().toLong()
            raw == Float::class.javaPrimitiveType || raw == Float::class.javaObjectType -> node.numberValue().toFloat()
            raw == Double::class.javaPrimitiveType || raw == Double::class.javaObjectType -> node.numberValue().toDouble()
            ...
            metadataResolver.isPersistable(raw) -> decodeObject(node, raw, context)
    ...
    private fun decodeObject(node: JsonNode, clazz: Class<*>, context: DeserializationContext): Any {
        ...
        val objectId = node.get(JsonCodec.ID_FIELD)?.asText()
    ...
    private fun resolveDocumentVersion(node: JsonNode, meta: PersistClassMeta): Int {
        val versionNode = node.get(JsonCodec.VERSION_FIELD)
        val documentVersion = when {
            versionNode == null || versionNode.isNull -> 1
            !versionNode.canConvertToInt() -> throw DeserializationException(
                "Document version field '${JsonCodec.VERSION_FIELD}' must be an integer for ${meta.clazz.name}"
            )
