...
internal object ReflectionPersistMetadataResolver : PersistMetadataResolver {
    private val reservedJsonNames = setOf(JsonCodec.ID_FIELD, JsonCodec.REF_FIELD, JsonCodec.VERSION_FIELD)
    private val cache = ConcurrentHashMap<Class<*>, PersistClassMeta>()

    override fun isPersistable(clazz: Class<*>): Boolean = clazz.isAnnotationPresent(Persistable::class.java)

    override fun requirePersistable(clazz: Class<*>) {
        if (!isPersistable(clazz)) {
            throw MissingPersistableAnnotationException(clazz.name)
        }
    }

    override fun getMeta(clazz: Class<*>): PersistClassMeta = cache.computeIfAbsent(clazz) { inspect(it) }
